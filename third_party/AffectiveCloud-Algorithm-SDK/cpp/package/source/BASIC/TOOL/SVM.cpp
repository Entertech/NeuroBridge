...
static std::string readline(std::string& input, int& pos)
{
	std::string line;
...
bool read_model_header(std::string& input, int& pos, svm_model* model)
{
	...
	std::string cmd = readline(input, pos);
	while(!cmd.empty())
	{
		...
		if(cmd == "svm_type")
		{
			...
			{
				if(strcmp(svm_type_table[i],cmd.c_str())==0)
				{
					param.svm_type=i;
					break;
				}
		...
		else if(cmd == "kernel_type")
		{
			...
			{
				if(strcmp(kernel_type_table[i],cmd.c_str())==0)
				{
					param.kernel_type=i;
					break;
				}
		...
		else if(cmd == "prob_density_marks")
		{
			int n = 10;    // nr_marks
			model->prob_density_marks = Malloc(double,n);
			for(int i=0;i<n;i++)
				iss >> model->prob_density_marks[i];
		}
		else if(cmd == "nr_sv")
		{
			int n = model->nr_class;
		...
		else if(cmd == "SV")
		{
			// while(1)
			// {
			// 	std::string line = readline(input, pos);
			// 	if(line.empty() || line == "\n") break;
			// }
			break;
		}
		...
		cmd = readline(input, pos);
	}

	return true;
}

svm_model *svm_load_model(std::string &model_file)
{
	if (model_file.empty())
		return NULL;
	int input_pos = 0;
	...
	// read header
	if (!read_model_header(model_file, input_pos, model))
	{
	...
	while(1)
	{
		auto lineStr = readline(model_file, input_pos);
		if (lineStr.empty() || lineStr == "\n") break;
		char *line = new char[lineStr.size() + 1];
		std::copy(lineStr.begin(), lineStr.end(), line);
		line[lineStr.size()] = '\0';
		p = strtok(line,":");
		while(1)
		{
