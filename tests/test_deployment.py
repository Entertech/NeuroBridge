...
class DeploymentTests(unittest.TestCase):
    def test_kylin_startup_entry_never_updates_source_and_opens_the_menu(self) -> None:
        ...
        self.assertIn("1. 一键准备并启动（默认开机自启，推荐）", menu_result.stdout)
    ...
    def test_kylin_gateway_assistant_guides_setup_and_repairs_git_permissions_safely(self) -> None:
        ...
        self.assertEqual(help_result.returncode, 0, help_result.stderr)
        self.assertIn("Prepare everything needed and start with boot autostart", help_result.stdout)
        ...
        self.assertEqual(menu_result.returncode, 0, menu_result.stderr)
        self.assertIn("1. 一键准备并启动（默认开机自启，推荐）", menu_result.stdout)
        self.assertIn("2. 直接启动网关（遵循已保存的自启配置）", menu_result.stdout)
        ...
        self.assertIn('autostart_preference_path="$runtime_dir/config/kylin-autostart.conf"', script)
        self.assertIn("默认使用 systemd 开机自启", script)
        self.assertIn("已显式配置为非自启", script)
    ...
    def test_kylin_autostart_runs_project_gateway_as_desktop_user(self) -> None:
        ...
        self.assertIn('preference_path="$runtime_dir/config/kylin-autostart.conf"', script)
        self.assertIn("configuredAutostart=%s", script)
        self.assertIn("write_autostart_preference false", script)
        self.assertIn("write_autostart_preference true", script)
        self.assertIn("Autostart is", help_result.stdout)
        self.assertIn("refusing to stop it", script)
        self.assertIn("is explicitly disabled", script)
    ...
    def test_kylin_usb_serial_diagnosis_prompts_times_out_and_preserves_logs(self) -> None:
        ...
        self.assertIn("project .runtime/logs/diagnostics", help_result.stdout)
        ...
        self.assertIn('default_output_dir="$root_dir/.runtime/logs/diagnostics"', script)
    ...
    def test_capture_viewer_uses_the_running_gateway_without_opening_serial(self) -> None:
        ...
        self.assertIn("耳机完整原始帧", html)
        self.assertIn("每行对应一个完整 28 字节串口帧", html)
        self.assertIn("页面按已确认串口结构还原完整 28 字节帧", html)
        self.assertIn("导出完整帧", html)
        self.assertIn("根据收到的 EEG/HR 原始负载按已确认串口结构还原", html)
        ...
        self.assertIn("const SERIAL_FRAME_BYTES = 28", javascript)
        self.assertIn("Uint8Array.of(0xAA, 0xAA, 0xAA)", javascript)
        self.assertIn("Uint8Array.of(0xBB, 0xBB, 0xBB)", javascript)
        self.assertIn("function restoreSerialFrame", javascript)
        self.assertIn("frame.set(SERIAL_FRAME_HEADER, 0)", javascript)
        self.assertIn("frame[3] = SERIAL_FRAME_BYTES", javascript)
        self.assertIn("frame.set(eegPacket, 4)", javascript)
        self.assertIn("frame[24] = hrPacket[0]", javascript)
        self.assertIn("frame.set(SERIAL_FRAME_TAIL, 25)", javascript)
        self.assertIn("appendRawRecord(serialFrame", javascript)
        self.assertNotIn("appendRawRecord(eegPacket", javascript)
        self.assertNotIn("appendRawRecord(hrPacket", javascript)
        self.assertIn("该窗口不生成完整串口原始帧", javascript)
        self.assertIn('Array.from(bytes, (value) => String(value)).join(" ")', javascript)
        self.assertIn('id="exportButton"', html)
        self.assertIn('id="sourceNotice"', html)
        ...
        self.assertIn("已收到耳机的有效脑电数据", javascript)
        ...
        self.assertNotIn("@media (min-width: 1180px)", styles)
        self.assertIn("收到有效脑电数据后显示已连接", javascript)
        self.assertIn("尚未收到有效脑电数据", javascript)
        self.assertNotIn("请从日志区分 ACK 未返回 01、算法未就绪或 E1 写入失败", javascript)
        self.assertIn("两项就绪后启动采集", html)
        self.assertNotIn("发送 ACK", html)
