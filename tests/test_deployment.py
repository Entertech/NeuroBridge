...
class DeploymentTests(unittest.TestCase):
    ...
    def test_capture_viewer_uses_the_running_gateway_without_opening_serial(self) -> None:
        ...
        self.assertIn("每行仅包含毫秒时间戳和网关实时收到或从历史录播读取的原始字节", html)
        ...
        self.assertIn("实时耳机未连接 · 当前数据为录播", javascript)
        self.assertIn("当前数据来源：历史录播", javascript)
        styles = (capture_root / "styles.css").read_text(encoding="utf-8")
        self.assertIn("repeat(auto-fit", styles)
        self.assertIn("minmax(min(100%, 430px), 1fr)", styles)
