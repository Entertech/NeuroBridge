...
id -u neurobridge >/dev/null 2>&1 || useradd --system --home /nonexistent --shell /usr/sbin/nologin neurobridge
...
if [[ ! -e "$config_dir/gateway.toml" ]]; then
  install -m 0640 "$install_dir/config/gateway.toml.example" "$config_dir/gateway.toml"
fi
