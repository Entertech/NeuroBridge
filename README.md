...
设备侧也使用独立策略：`data_source.type = "serial"` 选择当前 USB 派生 TTY 方案，`"bluetooth"` 切回 Flowtime BLE，一次只运行一种；`"usb"` 仅为原生 HID/Bulk/Interrupt 预留，协议未确认时会拒绝启动并提示改用串口。串口策略稳定排序并逐个探测 `/dev/serial/by-id/`、`ttyACM*`、`ttyUSB*`：网关主动发送固定 7 字节 ACK，首个返回独立单字节 `0x01` 的候选进入 `connected` 并停止遍历。设备不会主动发送握手协议；因此每次网关启动或重连都可重新执行 ACK → `0x01` 校验，不需要持久化 USB 身份，也不存在“服务重启后等不到设备主动握手”的恢复风险。ACK 校验成功后进入 `validating`，本地算法就绪即无响应发送 `0xE1`；设备只有收到 `0xE1` 才开始发送 28 字节脑电帧，网关收到首个合法帧后才进入 `validated`。`0xE0` 仅停止脑电数据，不表示断开。ACK 校验超时、算法未就绪、控制写失败或 `0xE1` 后首帧超时均进入 `validation_failed` 并记录脱敏阶段。该行为目前为源码与自动化测试支持，仍需目标麒麟机现场验证。银河麒麟一键流程使用项目内锁定的 CMake 3.31.6、目标系统 Eigen 3.3.7 与仓库源码，将 C++ 算法 bridge 构建到 `.runtime/algorithm/`，进程自检成功后才启用配置。16 位大端序号用于输出累计和区间丢包率，日志不记录控制响应原文或完整脑波帧。完整 28 字节设备帧只写入受保护的内部录制目录，不进入现有采集包、诊断包或北向消息。
...
1. 在最终 N100/N150 + 银河麒麟镜像上验证多 TTY 遍历、主动 ACK 后以首个独立单字节 `0x01` 短路选中目标、无响应 `0xE1`/`0xE0` 写入、`0xE1` 后首个 28 字节帧门禁、分帧、丢包统计、拔插重连和长时间运行。
