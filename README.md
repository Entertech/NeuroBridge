...
| Windows 网关 | [`windows/README.md`](windows/README.md)；[一键操作教程](doc/tech/麒麟V10网关运行与串口联调内部文档.md#101-一键准备与启动推荐) | 双击 `windows/neurobridge-windows-bootstrap.cmd` 即自动准备环境和配置并前台启动，无需输入菜单数字；需配套 Windows 算法程序。Windows 7 运行时、签名和实机验收未完成，不能作为交付入口。 |
