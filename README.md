...
设备侧也使用独立策略：`data_source.type = "serial"` 选择当前 USB 派生 TTY 方案，`"bluetooth"` 切回 Flowtime BLE，一次只运行一种；`"usb"` 仅为原生 HID/Bulk/Interrupt 预留，协议未确认时会拒绝启动并提示改用串口。串口策略会稳定排序并逐个探测 `/dev/serial/by-id/`、`ttyACM*`、`ttyUSB*`，收到首个固定握手后停止遍历并进入 `connected`，立即回写相同 7 字节 ACK；只有握手帧之外的独立单字节 `0x01` 才说明 ACK 校验成功。设备只在重新上电后发送握手，`0xE0` 只停止脑电数据、不表示断开；因此首次成功后会把严格 USB 身份保存到运行目录。网关异常退出或重启时先被动观察匹配设备：若已收到合法 28 字节脑电帧，直接接管当前数据流且不发送 `0xE1`；若设备静默，算法就绪后才发送无响应 `0xE1` 并等待首帧确认。未知设备不会收到控制命令。ACK 校验超时、算法未就绪、控制写失败或静默恢复首帧超时均进入 `validation_failed` 并记录脱敏阶段。该异常恢复目前为源码与自动化测试支持，仍需目标麒麟机现场验证。银河麒麟一键流程使用项目内锁定的 CMake 3.31.6、目标系统 Eigen 3.3.7 与仓库源码，将 C++ 算法 bridge 构建到 `.runtime/algorithm/`，进程自检成功后才启用配置。16 位大端序号用于输出累计和区间丢包率，日志不记录控制响应原文或完整脑波帧。完整 28 字节设备帧只写入受保护的内部录制目录，不进入现有采集包、诊断包或北向消息。
...
1. 在最终 N100/N150 + 银河麒麟镜像上验证多 TTY 遍历、首握手短路、握手 ACK 后单字节 `0x01` 校验、无响应 `0xE1`/`0xE0` 写入、分帧、丢包统计、拔插重连和长时间运行。
