...
网关所有输出统一为 `{protocolVersion, code, data, message}`：当前 `protocolVersion` 固定为 `1.0`，`code=200` 成功，非 `200` 时 `message` 为错误原因。连续数据事件的 `data` 中包含 `mode`（`live`/`replay`）、`deviceId`、`subjectId`、`gatewayId`、`sequence`、`timestampMs`、`valid` 与 `payload`。原始波形和节律数据使用批量 JSON 或二进制帧，避免单采样点逐条 JSON 发送。
