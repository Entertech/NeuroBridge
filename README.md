...
设备侧也使用独立策略：`data_source.type = "serial"` 选择当前 USB 派生 TTY 方案，`"bluetooth"` 切回 Flowtime BLE，一次只运行一种；`"usb"` 仅为原生 HID/Bulk/Interrupt 预留，协议未确认时会拒绝启动并提示改用串口。串口策略会稳定排序并逐个探测 `/dev/serial/by-id/`、`ttyACM*`、`ttyUSB*`，收到首个固定握手后停止遍历并进入 `connected`，立即回写相同 7 字节 ACK；ACK 等待窗口内若设备又发出完整周期握手，网关会重新回 ACK 并继续等待，只有握手帧之外的独立单字节 `0x01` 才说明 ACK 校验成功。随后本地算法必须准备成功，网关才发送无响应的单字节 `0xE1` 并进入 `validated`、读取和转发实时数据；停止时发送同样无响应的 `0xE0`。ACK 校验超时、算法未就绪或控制写失败均进入 `validation_failed` 并记录脱敏阶段；关闭串口后为 `disconnected`，从未找到目标设备则回到 `not_connected`。银河麒麟一键流程使用项目内锁定的 CMake 3.31.6、目标系统 Eigen 3.3.7 与仓库源码，将 C++ 算法 bridge 构建到 `.runtime/algorithm/`，进程自检成功后才启用配置。16 位大端序号用于输出累计和区间丢包率，日志不记录控制响应原文或完整脑波帧。完整 28 字节设备帧只写入受保护的内部录制目录，不进入现有采集包、诊断包或北向消息。
