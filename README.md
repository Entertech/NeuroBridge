...
- `web/`：纯静态网页；`capture/` 是采集控制台和网关本机配置页，`b-client-test/` 是 B 端联调页，可由任意平台的本地服务或静态服务器托管。
...
| 采集控制台网页 | [`web/capture/`](web/capture/) | 平台无关静态资源；macOS POC 挂载到 `/capture/`，Ubuntu 网关将其由仅回环的 `neurobridge-config-ui` 挂载，并提供本机配置接口。 |
