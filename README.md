...
需要远程运维时，Ubuntu 环境准备会安装但不启用 OpenSSH；部署后执行 `sudo ./linux/setup-ssh-operations.sh` 即可交互式完成运维账号密码、私有监听地址和来源网段配置。现场只使用一个受信任的 SSH 运维账号：该账号可上传完整版本到 `/srv/neurobridge-release`，再用 `neurobridge-ops update` 一键更新和重启；还可使用 `status`、`logs --follow`、`audit` 和 `restart`。`update` 不自行联网拉取代码，`audit` 不记录密码或业务原始数据。由于该账号提供的代码随后会以 root 安装，必须把它作为网关管理员凭据管理。完整命令和回滚步骤见 [Ubuntu 网关部署与运行教程](linux/README.md#41-启用-ssh-运维入口可选) 与 [SSH 运维和录播联调指南](doc/tech/网关%20SSH%20运维与%20B%20端录播联调指南.md)。
