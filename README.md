...
- 演示范围：本次为单人，但北向协议必须保留 `deviceId` 与 `subjectId`。
...
首期北向传输统一采用 **WSS/WebSocket + UTF-8 JSON**：网关作为服务端，B 端主动连接并鉴权；连续数据最多每 600 ms 推送一组。请求、数据结构、BLE 字节映射、算法字段、错误码与验收项见《[头环蓝牙网关北向网络协议 v0.1](doc/tech/%E5%A4%B4%E7%8E%AF%E8%93%9D%E7%89%99%E7%BD%91%E5%85%B3%E5%8C%97%E5%90%91%E7%BD%91%E7%BB%9C%E5%8D%8F%E8%AE%AE_v0.1.md)》。
...
每条数据至少应包含 `messageType`、`mode`（`live`/`replay`）、`deviceId`、`subjectId`、`gatewayId`、`sequence`、`timestampMs`、`data` 与 `valid`。原始波形和节律数据使用批量 JSON 或二进制帧，避免单采样点逐条 JSON 发送。
...
- 使用有线以太网连接第三方主机；联调前确认地址、端口、鉴权、连接方向、ACL/VLAN/防火墙与断线补传规则。
