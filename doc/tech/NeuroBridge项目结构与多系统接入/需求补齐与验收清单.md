...
2026-09-09 现场反馈补充：用户确认 Windows 10 专业版 22H2（OS 内部版本 19045.6466，x64；Inspiron 7559、i7-6700HQ、16 GB RAM）和银河麒麟 V10 SP1 2503 均运行联调成功，见[已验证环境记录](../麒麟V10网关运行与串口联调内部文档.md#已验证环境2026-09-09-用户确认)。以下“本轮未验收”等条目保留为 2026-09-08 的源码核对结果；本次反馈确认两种系统运行成功，但未提供逐项测试证据，不自动关闭算法结果比对、异常恢复、禁录播、24 小时长稳或正式发布门禁。
...
| AC-004 静默直接 E1/完整帧验证 | §5、§8 | 串口发现与算法准备并行；Source 在两项就绪后 E1，仅合法帧可验证；准备异常不使能静默设备 |
...
| persistence_timeout_ms | 1000 | 单次确认等待毫秒；覆盖 Windows 正常 flush/fsync 延迟，超时仍计入持久化缺口 |
...
stateDiagram-v2
    ...
    connecting --> validating: 静默窗口结束，等待算法 ready 后 E1
    validating --> validated: 收到完整合法 28 字节帧
    validating --> validation_failed: 首帧超时且无候选成功
...
| 设备连接 | 模拟无 ACK 的 E1/完整帧验证/E0、分帧和重连逻辑 | 真实 USB 枚举、电气行为、拔插 |
| 数据/算法 | 合成数据、超时、积压、持久化故障 | 真实算法基线、性能阈值、24 小时长稳 |
| 浏览器订阅 | WS 合同、Origin、600 ms 和重连顺序 | 目标机浏览器页面体验 |
| 录播分支 | Serial 禁播和 409 合同 | BLE 头环录播现场回归 |


## 直接 E1 启动补充（2026-09-10）

设备端经用户确认：耳机首次上电未握手也可直接 E1 出数。当前实现统一 Windows/麒麟串口发现与算法准备并行、静默候选直接 E1、完整合法帧验证，移除 ACK 和停止提示探测。此前握手行为确认记录保留在版本台账，由本次实现取代旧策略。

| 场景 | 预期与验证边界 |
|---|---|
| 首次上电 / E0 后服务或整机重启 | 不发送 ACK；两项就绪后 E1，收到完整合法帧才验证 |
| 已有合法流 | 跳过 E1，原始数据路径允许算法不可用并报告状态 |
| 串口或算法先就绪 | 独立并行进行；算法失败不向静默设备发 E1 |
| 无帧 / 独立 01 / 非法数据 | 不创建录制会话、不输出有效数据，不启用历史录播 |
| 失败 / 取消 / 多候选 | 尝试 E1 的候选尽力 E0，等待 I/O 线程收尾并关闭，回收预备算法 |
| 成功接管 | 复用算法且不重复初始化/E1，按读取时间保存首帧 |

`tests/test_serial_startup.py` 覆盖上述主要源码场景并接入 Windows CI。Windows 和麒麟各自的真实设备冷启动、拔插、服务/整机重启、离线拒绝录播及长稳仍待现场复测，不把旧版本一般联调成功计入新流程验收。
