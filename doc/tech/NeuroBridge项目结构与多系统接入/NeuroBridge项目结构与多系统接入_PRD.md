...
本文后续所称“银河麒麟 V10”覆盖服务器版和桌面版，以及 x86_64、ARM64、LoongArch64、MIPS64el、SW64 五类架构。2026-09-07 实机诊断观测到桌面版 SP1 2503 和 systemd 245；各发行版/架构的正式 ISO、ABI、包管理器行为和安装路径由真机验证日志补齐。
...
| 银河麒麟 V10 服务器版/桌面版，x86_64、ARM64、LoongArch64、MIPS64el、SW64 | 耳机 | USB 派生 TTY 串口 | 本机页面，`access.mode="local_browser"`，仅回环地址 | 本次发布矩阵；仅支持实时数据，不支持录播；每个组合交付 DEB/RPM 双格式 |
| Windows 7/10/11，x86/x86_64 | 耳机 | USB 虚拟串口（COM） | 本机页面，`access.mode="local_browser"`，仅回环地址 | 本次发布矩阵；仅支持实时数据，不支持录播；每个组合交付 EXE/MSI 双格式 |
...
- 未完成目标系统真机验证前的发布状态只能标记为 `candidate`；Windows Profile 及其串口数据源属于本次矩阵，不能以源码支持代替现场验收；
...
5. 最后完成未签名正式安装包、SBOM、构建清单、GitHub Release、tag 和 Workflow 发布门禁；签名留作后续变更。
...
|---|---|---|---|
| 银河麒麟 V10 服务器版/桌面版，x86_64、ARM64、LoongArch64、MIPS64el、SW64 | DEB 与 RPM 双格式 | 暂无 | 安装网关程序、算法 bridge、本机网页、配置、systemd 服务和运维工具 |
| Windows 7/10/11，x86/x86_64 | 未签名 MSI | 未签名 EXE 安装器 | 安装网关程序、算法 bridge、本机网页、配置和 Windows Service |

本次发布工作流明确交付 DEB 与 RPM 两种格式。每个麒麟发行版/架构组合都必须分别生成、安装、升级和卸载验证；目标镜像的实际包管理器差异记录在验证日志中。代码签名、安装包签名和公证暂不属于本次门禁。
...
- 安装程序校验对应系统版本、目标架构、磁盘空间、串口运行权限和包完整性；
...
flowchart TD
    ...
    G --> K[银河麒麟服务器/桌面 V10 × 五架构]
    G --> W[Windows 7/10/11 × x86/x86_64]
...
- 银河麒麟安装包和算法 bridge 必须按服务器/桌面及五种架构使用对应的受控 runner、构建机或已验证交叉工具链生成；Ubuntu runner 的构建结果不能替代目标包验证；
- Windows 安装包和算法 bridge 必须覆盖 Windows 7/10/11 与 x86/x86_64；Windows 7 兼容性不能仅由较新 Windows runner 的构建成功代替，必须在 Windows 7 目标镜像或实机上运行安装与长稳验收；
...
完整离线运行时、五架构/双发行版干净机升级回滚验收，以及 Windows 7 的 Python/服务运行时和补丁基线仍需通过真机验证日志补齐。签名暂不属于当前发布门禁；当前分支仍只有候选包源码支持，不代表真机交付已完成。
