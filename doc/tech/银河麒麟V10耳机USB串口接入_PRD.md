...
打开候选串口
  ...
  └─ 观察窗口内没有合法帧
       └─ 进入 validating → 写入固定 ACK AA 55 01 01 01 01 6F
...
- 只有在候选成功打开且完整观察窗口内没有合法 28 字节帧时才发送 ACK；发现合法流时禁止发送。
...
- 唯一候选 ACK 超时或所有已打开候选均未返回独立 `0x01` 时，状态保持 `validation_failed`；只有下一轮重试开始时才进入 `connecting`，不得改写成普通 `not_connected`/`disconnected`。
