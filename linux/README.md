...
配置器会隐藏输入并要求再次确认正好 6 位数字的运维账户密码。密码不会出现在命令行、配置文件或日志中。配置器仅为该专用运维账户显式选择系统 `chpasswd` 支持的 SHA512 加盐哈希写入方式，从而不经过 PAM 的最短 8 位检查；不会修改整机 PAM 密码策略，其他账户不受影响。若要在自动化部署中使用非交互方式，通过标准输入提供密码；以下 IP 和网段只是示例，必须替换为现场确认值：
