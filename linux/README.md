...
也可在**网关本机桌面**打开 `http://127.0.0.1:8090/`。该本地控制台读取同一个配置文件，保存前会校验字段并重启网关；地址、网段、网卡或 DHCP 发生变化时，必须勾选确认后才会应用既有的安全 Netplan 配置流程。该服务固定只监听回环地址，B 端主机和任何其他网络均无法访问；若页面显示保存、Netplan 或服务重启错误，应按完整错误信息修正后重试，或继续使用本节的脚本方式处理。
