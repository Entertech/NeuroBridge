...
- 若要使用 SSH 运维，确认运维账户名、至少 6 位数字的密码、运维主机固定 IP/CIDR，以及网关用于运维的私有静态 IP。密码只在网关本地控制台隐藏输入，不应写入现场文档、命令历史或日志。数字密码正式部署仅适用于专用网线直连和固定来源 IP 的封闭现场；短时局域网验证必须限制为受控网络和单机 `/32` 来源，禁止把 SSH 接入公网、无线网或不受控局域网。
...
常规单网卡、单 B 端场景，在网关的**本地控制台**使用快速模式；它固定使用账号 `neuroops`、端口 `22`、检测到的当前网关私有 IP，并把给定 B 端 IP 自动限制为 `/32`：

```bash
sudo ./linux/setup-ssh-operations.sh --quick 192.168.88.20
```

也可省略命令中的 B 端 IP，让脚本单独询问。密码仍需隐藏输入并二次确认，最终确认仍保留。检测不到网关私有 IP、网关存在多块私网网卡，或需要自定义账号、监听地址、来源网段和端口时，使用保留的完整交互模式：
