...
- 录播由网关部署配置管理，浏览器不选择录播或切换数据源；只有明确声明 `supports_replay=true` 的蓝牙头环兼容 Profile 才允许在 `subscribe` 或 `getLatest` 后使用录播。耳机串口 Profile 固定 `supports_replay=false`，配置不得填写 `replay_recording_id`，实时路径未验证或断开时不得读取任何历史录制，不得创建录播任务或输出 `mode="replay"`；银河麒麟当前实现返回 `409 STREAM_NOT_AVAILABLE_REASON`，恢复串口后才继续实时采集。
...
- 当前设备依据军工耳机脑电串口协议修订号 181：115200 8-N-1、固定 28 字节帧。2026-09-10 用户确认首次上电未握手也可直接 E1 出数。Windows 与麒麟统一移除 ACK/独立 `0x01` 验证和停止提示文件；设备发现与算法准备并行，候选已打开且算法 ready 后无响应发送 `0xE1`，仅完整合法帧能验证连接。已有合法流直接接管，不重复 E1；算法不可用仍可保存已有流原始数据。
- 验证前不创建录制会话；验证后复用预备算法并原样保存完整帧及读取时间。E1 写成功或单独 `0x01` 不代表已连接。正常停止最多尝试一次无响应 `0xE0`；失败/取消探测若已尝试 E1，尽力 E0 再关闭候选并清理算法。两平台各自完成真实冷启动、停止/整机重启与离线不录播验收，不以设备端确认或模拟测试代替。
...
- 修改设备接入、算法或网络依赖后，必须在银河麒麟 V10 上验证实时采集、串口离线及存在历史录制时仍不启动录播、耳机拔插/重连、服务重启和浏览器断开/恢复场景。
