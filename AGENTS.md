...
- **默认只记录，不改对外文档：**除非用户在当前请求中明确要求“更新对外文档”或等效授权，否则不得修改台账指定的 B 端 Markdown/PDF，也不得更新其版本、发布日期或摘要；只在台账 `change_records` 与 `doc/tech/版本变更记录.md` 记录变更内容、影响和未发布原因。对外版本更新必须同时落盘：台账版本/日期/摘要与一条标记为 `explicit_user_authorized` 的变更记录、对应 Markdown 和 PDF；随后运行 `tools/check-external-protocol.sh`。历史版本和内部实现基线默认不改。若版本关系、签字状态、受众或授权无法从仓库确定，停止对协议文件的写入并请求用户确认。
