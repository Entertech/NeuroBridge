...
- **协议文档发布门禁（每次修改协议、README 中的协议链接或生成协议 PDF 前必须执行）：**先用 `rg --files doc/tech` 查找同主题的所有协议版本，逐一确认文档首页的受众、状态、版本关系和旧版本的迁移说明；再明确本次改动的交付对象。不得仅因旧版被列为实现依据或被需求文档引用，就将它当作当前对外协议。只有被标记为当前 B 端接入/双方签字基线的版本可以作为对外交付物修改和生成 PDF；历史版本和内部实现基线默认不改。若版本关系、签字状态或受众无法从仓库确定，停止对协议文件的写入并请求用户确认。
