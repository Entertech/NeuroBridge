...
本仓库实现头环设备数据到本机可视化前端或兼容第三方 B 端的 Linux/PC 网关。当前设备接入需求以[多传输数据源接入 PRD](doc/tech/%E5%A4%B4%E7%8E%AF%E7%BD%91%E5%85%B3%E5%A4%9A%E4%BC%A0%E8%BE%93%E6%95%B0%E6%8D%AE%E6%BA%90%E6%8E%A5%E5%85%A5_PRD.md)为准；历史背景见[对接方案 v0.1](doc/tech/%E5%A4%B4%E7%8E%AF%E8%93%9D%E7%89%99%E7%BD%91%E5%85%B3%E5%AF%B9%E6%8E%A5%E6%96%B9%E6%A1%88_v0.1.md)。README 仅作项目入口和摘要；报文语义仍以双方最终签字的北向协议为准。

## 架构边界

- 当前默认链路为：头环设备 → 本项目网关后台 → `127.0.0.1` 回环 WebSocket → 同机浏览器，不需要 B 端专用网线。兼容策略可切换为“网关 → 专用有线网络 → 第三方 B 端主机”。
- 本机浏览器与兼容有线 B 端必须通过策略模式复用同一业务契约；策略只决定监听、网页服务、来源校验和部署网络，不得在业务层散布模式分支。
- 网关拥有设备连接、配对、重连、数据解析、时间戳、缓存、算法调用、录播和北向协议适配。
- 默认由同机浏览器负责展示，网关继续负责持久化；兼容模式下第三方主机可展示与保存。本项目不负责其后续服务端上传。
- 首期正式部署目标为银河麒麟 V10 on x86_64（N100/N150），Ubuntu LTS 作为兼容与回归基线。浏览器只是前端，不得把网关的设备、算法和持久化职责迁入 Web Serial 或其他浏览器硬件 API。
...
- 数据通道固定为网关服务端、浏览器/客户端的 WS/WebSocket + UTF-8 JSON。默认 `local_browser` 仅监听 `127.0.0.1`、提供本机 HTTP 页面并校验固定 Origin；兼容 `wired_b_side` 才使用专用有线网络和私有静态地址。两种策略不改变报文版本和动作语义，禁止暴露到公网或不受控网络。
...
- 首期设备 profile 以 [Flowtime headband 蓝牙通信协议](https://entertech.feishu.cn/docs/doccnlmMLpxwY25gJQyiFQmBeRd?from=from_copylink) 为准：`FF31` 为 20 字节大端 EEG 包（2 字节序号和 6 个 24 位采样），`FF51` 为 1 字节心率包，`FF32` 为 2 字节 EEG/HR 佩戴与接触状态；该协议未定义 `FF52`，不得订阅或假定它存在。采集控制写 `FF21` 的 `0x05` 启动、`0x06` 停止。所有已定义原始字节必须持久化，不得被 SDK 解包示例改写。
...
- 浏览器/客户端连接断开、头环离线、非法数据、算法异常、网络不可达和服务端拒绝均应转换为可观测状态，不能让主进程退出。
- 重连、缓存上限、补传、幂等键和日志轮转必须配置化。缓存或补传是否启用须遵守双方协议。
- 服务以 systemd 开机自启；日志应包含网关版本、头环连接状态、数据时间范围和错误原因，但不得记录令牌、密码、私钥或完整敏感原始数据。
- 修改设备接入、算法或网络依赖后，至少验证：实时、录播、设备断线重连、浏览器/客户端断开与恢复场景；默认验证同机浏览器，`wired_b_side` 才验证独立 B 端和专网恢复。
