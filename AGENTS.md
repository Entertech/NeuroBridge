...
- Windows 耳机重启恢复采用用户确认的 ACK/E1 探测：先观察已有合法流；同一 USB/COM 身份存在正常 E0 停止记录时先 E1，无合法帧再 ACK，否则先 ACK、超时再 E1。E1 前准备隔离算法，E1 不等待命令应答，必须观察到完整合法 28 字节帧才验证；停止记录只选择顺序，不能替代设备验证。失败/取消探测尽力 E0 关闭，后续按配置重试。该 Windows 恢复规则不改变麒麟当前握手策略。
